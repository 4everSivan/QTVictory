...
**QTVictory 0.1.0** 是首个正式功能基线版本（**Phase 1 · 模拟底座与观察员控制台**）及其维护期演进。系统确立纯私人、单机自部署定位的 A 股实时前向仿真模拟交易系统，完整实现规则级撮合引擎、计划引擎、多交易员实例管理体系以及基于 Nothing 设计系统的现代化 Web 监控控制台。
...
### 维护与修复 (Fixed)
- **修复 CI E2E 端口环境变量注入**（[C026](docs/devel/change/C026-修复CI-E2E端口环境变量注入.md)）：GitHub Actions E2E 任务因 Vite `createServer()` 未在 CI 环境正确加载 `frontend/.env` 中的 `VITE_API_BASE`，前端回退至默认端口 8787 而后端运行在 8788，导致交易员列表加载失败、`TraderSwitcher` 渲染空态、`page.$eval('.switcher-name')` 崩溃。修复：`e2e.mjs` 从 `QTV_PORT` 显式推导 `apiBase` 并通过 Vite `define` 选项编译时注入 `import.meta.env.VITE_API_BASE`，消除对 `.env` 文件加载的隐式依赖；CI 工作流 E2E 步骤改用 `env:` 块显式声明环境变量，新增失败日志转储步骤。详见变更卡 [C026](docs/devel/change/C026-修复CI-E2E端口环境变量注入.md)。
- **治理自检脚本取证失效与门禁逃逸缺陷修复（21 项）**（[C023](docs/devel/change/C023-修复治理自检脚本取证失效与门禁逃逸缺陷.md)）：`scripts/jev_workflow_check.py` 首次作为红线 7 门禁投入实战后，经对抗性复查 + 本地实证发现 21 项缺陷并一次修完。无声失效类：零沉淀账本正则漏配复合流转 `EN-0002/EN-0003 已流转至 [C002]/[C003]`，EN-0002 整个从账本消失、EN-0003 被错配，`sediment_violations` 静默放行；`--offline` 跳过代码侧硬判，非法状态只打印不门禁、退出码仍为 0；`sync_complete` 取证指令引用不存在的 `latest_change_card`；`call_jev` 读体/解码/解析异常逃逸成契约外 exit 1；`validate_response` 不校验置信与概率类型，畸形响应留到打印阶段崩溃。误拦类：`cross_check` 逐事实独立设约束，把本仓库常态（新卡待核验 + 上一卡已核验未合入）判成矛盾，3 次实跑全部误拦 exit 4；低置信语义判定被当作硬判据（实跑置信 0.30~0.52 照样拦）；noul 型答案无 `confidence` 字段却永久打「⚠ 低置信」噪音；noul 判罚用裸 `p < 0.5`、不看展示层尊重的 `NOUL_UNCERTAIN_BAND`，同一份概率在答案行写「不确定」而结论行写「仍有同步点缺失」，自相矛盾；不确定带 ±0.15 对模型噪声过窄，同输入 4 次实跑 P=0.33~0.42 横跨下沿，干净仓库被误拦 exit 4。修复：流转账本改 ID 组/链接组按位置配对（不漏记）；offline 同权执行代码硬判；新增 `expected_stage()` 按最早未完成步骤推唯一期望阶段；`semantic_review_items()` 把未过 0.60 置信门禁的判定与落入不确定带的 noul 一并降级为人工复核（退出码 6），`detect_violation()` 只把过门禁的判定当硬判据；noul 门禁与展示层同源，带宽放宽至 ±0.25（下沿落在干净噪声上界 0.42 与真实违规 0.06 之间的空档）；新增 `frozen_dir_dirty` 断言让红线 2 可确定性判定；`unmerged_cards` 判别从字面「待提交」改为非 commit hash 即未合入；凭据解析兼容 `export` 前缀；git 输出显式 utf-8。回归保护：`scripts/tests/` 新增 153 项单测。详见变更卡 [C023](docs/devel/change/C023-修复治理自检脚本取证失效与门禁逃逸缺陷.md)。
- **治理自检交叉校验误拦「已全部闭环」态**（[C024](docs/devel/change/C024-修复自检交叉校验误拦已同步态.md)）：C022 回填 `关联提交` hash 后 `unmerged_cards` 转空，联网自检返回 exit 4 报「阶段判定与代码事实矛盾」——逐项核查 23 张卡的 hash 存在性、CHANGELOG 条目、design 回链、零沉淀与封存目录均无缺口，Jev 自身 compliance 也判「整体合规，个别待办」，属纯误拦。根因：`cross_check()` 把 `awaiting_bidirectional_sync` 的放行权绑在 `unmerged_cards` 非空上，而「同步是否闭环」是代码侧两个方向都裁不了的语义判断（design 原位回链 / 头部关联变更 / CHANGELOG 互链都要读正文才能确认），该条件于是形成反向漏洞——所有卡合入后 `expected_stage()` 退化为 `idle`，Jev 一旦对同步闭环猜疑就被升级成硬违规，而 `unmerged_cards` 为空正是每次正常收口后的必经路径，等于每个干净检查点都被误拦一次（干净检查点被拦会训练运营者忽略退出码，比漏拦更危险）。修复：放行集改为 `{expected, "has_violation", "awaiting_bidirectional_sync"}`，该标签与 `has_violation` 同列「代码侧无法否证」，其余三个标签的代码可否证约束不变。门禁强度未削弱——真实未闭环仍由 `sync_complete` noul 门禁与 compliance 档位闸独立兜住：注入 `EN-0002` 的仓库副本联网实跑仍 exit 4，且四条独立理由同时命中。回归保护：新增 4 项单测（含 main 级端到端回归），套件 157 项通过。详见变更卡 [C024](docs/devel/change/C024-修复自检交叉校验误拦已同步态.md)。
...
- **下单 `marketType` 显式 null/缺省被 422 拒绝**：契约容忍并归一 `best5_cancel`（限价单不落类型），市价单缺省/显式 null 均 201 正常落单。详见变更卡 [C016](docs/devel/change/C016-修复下单marketType空值422.md)。
...
- **开发计划与任务卡**：19 篇里程碑计划（`docs/archive/v0.1.0/plan/M1–M18`）与 27 篇任务细化卡（`docs/archive/v0.1.0/task/T01–T27`）。
- **终验报告**：[01-M8 后端验收报告.md](docs/archive/v0.1.0/report/01-M8-验收报告.md)、[02-M16 前端验收报告.md](docs/archive/v0.1.0/report/02-M16-前端验收报告.md)、[03-M17 自选股编辑验收报告.md](docs/archive/v0.1.0/report/03-M17-自选股编辑验收报告.md) 与 [04-M18 K线分析工具验收报告.md](docs/archive/v0.1.0/report/04-M18-K线分析工具验收报告.md)。
