...
8. **自检结果必呈人工**：凡调用治理自检脚本（含 `--offline` 与无 key 降级场景），必须将原始输出与退出码如实完整呈报人工，非 0 退出码须附处置说明；严禁仅以"已自检 / 通过"概括或隐瞒结果。
...
- **低置信需人工复核**：任一 choice 型判定（当前阶段 / 整体合规）置信低于 0.60 时，该判定降级为参考意见、不作为硬判据；noul 型判定无置信字段，概率落入不确定带（|P−0.5| < 0.25）时同样只作参考、不写成确定性结论。此时结论仅供参考，须人工核对采集事实后决策。
- 只采集状态、不调用模型：`python3 scripts/jev_workflow_check.py --offline`。
- **退出码语义**：`0` 未发现确定性违规；`2` 无 key；`3` API 调用失败；`4` 存在治理违规（不得合入主干，先修复再重跑）；`5` Jev 响应无效（须修复后重跑）；`6` 语义判定不确定或置信不足（结论仅供参考，须人工核对采集事实后决策；人工确认前不得视为已完成自检）。
- **结果必呈人工（红线 8）**：每次调用脚本后，AI 须在回复中如实呈报采集事实、语义判定、退出码与处置说明（含 `--offline` 与 exit 2 降级场景）；不得以"已跑过 / 已通过"概括，严禁隐瞒非 0 退出码。
