...
> **任务ID**: T16 ｜ **所属里程碑**: [M10-数据层与API客户端](../plan/M10-数据层与API客户端.md) ｜ **状态**: 已完成
> **前置依赖**: [T15-工程骨架与样式基线](T15-工程骨架与样式基线.md)；后端接口事实源 [T09-REST与鉴权幂等](T09-REST与鉴权幂等.md)、[T10-WebSocket与实时推送](T10-WebSocket与实时推送.md)（均已完成） ｜ **后置下游**: [T17](T17-行情面板.md)、[T18](T18-图表组件.md)、[T19](T19-交易员模块.md)（全部展示层）
> **设计依据**: [01-前端设计方案](../../../devel/design/01-前端设计方案.md) §6（错误码呈现）、§7（数据层全章）、§9（服务端状态缓存） ｜ **最后更新**: 2026-09-17
