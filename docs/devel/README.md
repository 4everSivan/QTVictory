...
- **[plan/](../archive/v0.1.0/plan/README.md)** —— 开发计划，承载 Phase 1 阶段流程总述、里程碑（M 编号）与进度矩阵（已归档至 v0.1.0）；
- **[task/](../archive/v0.1.0/task/README.md)** —— 任务细化卡（T 编号），每模块一份，是具体执行单元（已归档至 v0.1.0）；
- **[report/](../archive/v0.1.0/report/README.md)** —— 阶段验收报告与机器证据记录（已归档至 v0.1.0）；
- **[env/](env/README.md)** —— 开发环境配置、缓存盘点与安全清理指南；
- **[assessment/](assessment/README.md)** —— AI Agent 研发质量量化评估体系（评分规则常驻，报告已归档至 v0.1.0）；
- **[todo/](todo/README.md)** —— 待办与需求缓冲池，未入轨缺陷与需求的登记表（落地即移出，零沉淀）。

> 📌 **归档约定**：版本 tag 打出后，plan/、task/、report/ 整体以及 assessment/ 下的评分报告按 [docs/archive/README.md](../archive/README.md) 的 SOP 迁移至 `docs/archive/<版本号>/` 封存（只读）；Phase 1 文档已整体归档至 `docs/archive/v0.1.0/`；评分规则（01/02）与活文档（design/change/todo/env）常驻 devel 持续演进；Phase 2 立项时在 `docs/devel/` 下重建 `plan/` 与 `task/`。

当前状态：Phase 1 模拟底座与观察员控制台双线均已收口并通过终验，相关文档已归档至 `docs/archive/v0.1.0/`。项目总览见根 [README.md](../../README.md)。
...
| 目录 | 索引文档 | 核心职责 | 现行状态 |
|---|---|---|---|
| **design/** | [design/README.md](design/README.md) | 包含 [01-前端设计方案](design/01-前端设计方案.md) (v6.2) 与 [02-后端设计方案](design/02-后端设计方案.md) (v7.0)，为系统现行真理 | 现行基线 |
| **change/** | [change/README.md](change/README.md) | 承载 Bug 修复与回调的核验流，提供 [template.md](change/template.md) 变更卡模板与前后对比 Checklist | 持续演进 |
| **plan/** | [../archive/v0.1.0/plan/README.md](../archive/v0.1.0/plan/README.md) | Phase 1 开发计划与 M1–M18 里程碑（已归档；Phase 2 立项时重建） | 已归档 (v0.1.0) |
| **task/** | [../archive/v0.1.0/task/README.md](../archive/v0.1.0/task/README.md) | Phase 1 T01–T27 共 27 篇任务细化规格卡（已归档；Phase 2 立项时重建） | 已归档 (v0.1.0) |
| **report/** | [../archive/v0.1.0/report/README.md](../archive/v0.1.0/report/README.md) | Phase 1 阶段终验报告（01–04 已归档） | 已归档 (v0.1.0) |
| **env/** | [env/README.md](env/README.md) | 包含 [01-环境缓存与依赖清理指南](env/01-环境缓存与依赖清理指南.md)，指导磁盘与缓存治理 | 现行有效 |
| **assessment/** | [assessment/README.md](assessment/README.md) | 包含评分标准、执行流程（通用规则常驻）与 AI Agent 研发质量评估报告（随版本归档） | 规则现行 / 报告已归档 |
...
- **想要查验历史交付质量与测试证据**：请查阅 [docs/archive/v0.1.0/report/](../archive/v0.1.0/report/README.md) 中的阶段验收报告或 [docs/archive/v0.1.0/assessment/](../archive/v0.1.0/assessment/03-评分报告.md) 中的评估报告；
- **想要定位特定模块的历史任务定义**：请通过 [docs/archive/v0.1.0/plan/](../archive/v0.1.0/plan/README.md) 的追踪矩阵跳转到对应的 [docs/archive/v0.1.0/task/](../archive/v0.1.0/task/README.md) 任务卡；
