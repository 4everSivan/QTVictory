    ...
    async def test_trigger_to_fill_to_event(self):
        ...
        try:
            ...
            plan = await ctx.plans.create_plan({
                "traderId": tid, "name": "低吸", "scope": {"codes": ["600519"]},
            })
...
class TestStrategyRuntime:
    async def test_daily_schedule_fires_once(self):
        ...
        try:
            ...
            ctx.market.sync_klines([
                ("600519", f"2026-08-{d:02d}", 10.0, 10.0, 10.1, 9.9, 10000)
                for d in range(1, 29)
            ] + [("600519", f"2026-09-{d:02d}", 10.2, 10.4, 10.5, 10.1, 12000)
                 for d in range(1, 16)])
            await ctx.plans.create_plan({
                "traderId": tid, "scope": {"codes": ["600519"]},
    ...
    async def test_paused_trader_produces_nothing(self):
        ...
        try:
            ...
            tid = t["id"]
            await ctx.plans.create_plan({"traderId": tid, "scope": {"codes": ["600519"]}})
